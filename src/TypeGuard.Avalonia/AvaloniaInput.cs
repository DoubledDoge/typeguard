...
/// <summary>
/// Provides Avalonia-based input by reading the current text value from a <see cref="TextBox"/>.
/// </summary>
...
public class AvaloniaInput(TextBox textBox) : IInputProvider
{
    ...
    /// via <see cref="Dispatcher.Invoke{T}(Func{T})"/>.
    /// </remarks>
    /// <returns>The trimmed text value of the text box, or null if the text box has no text.</returns>
    public string? GetInput()
    {
        string? text = Dispatcher.UIThread.CheckAccess()
            ? _textBox.Text
            : Dispatcher.UIThread.Invoke(() => _textBox.Text);
