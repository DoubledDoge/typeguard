	...
	)
{
	...
	)
	{
		...
		return allowedValues.Length == 0
			...
			: AddRule(new AllowedValuesRule(allowedValues, customMessage));
	}
	...
	)
	{
		...
		return excludedValues.Length == 0
			...
			: AddRule(new ExcludedValuesRule(excludedValues, customMessage));
	}
	...
	public StringInputBuilder WithCustomRule(Func<string, bool> predicate, string errorMessage)
	{
		...
		return AddRule(new CustomRule<string>(predicate, errorMessage));
	}
}
