...
///     Defaults to constructing a standard <see cref="EnumHandler{TEnum}" /> from the provided
///     providers.
/// </param>
public class EnumInputBuilder<TEnum>(
	...
	: BuilderBase<TEnum, EnumInputBuilder<TEnum>>(
		(validatorFactory ?? ((p, ig, i, o) => new(i, o, p, ig)))(
	...
	where TEnum : struct, Enum
{
	...
	public EnumInputBuilder<TEnum> WithDefined(string? customMessage = null) =>
		AddRule(new DefinedEnumRule<TEnum>(customMessage));
	...
	public EnumInputBuilder<TEnum> WithNotDefault(string? customMessage = null) =>
		AddRule(new NotDefaultEnumRule<TEnum>(customMessage));
	...
	)
	{
		...
		return !enumerable.Any()
			...
			: AddRule(new AllowedEnumValuesRule<TEnum>(enumerable, customMessage));
	}
	...
	)
	{
		...
		return !enumerable.Any()
			...
			: AddRule(new ExcludedEnumValuesRule<TEnum>(enumerable, customMessage));
	}
	...
	public EnumInputBuilder<TEnum> WithHasFlag(TEnum requiredFlag, string? customMessage = null) =>
		AddRule(new HasFlagRule<TEnum>(requiredFlag, customMessage));
	...
	public EnumInputBuilder<TEnum> WithNotHasFlag(
		TEnum forbiddenFlag,
		string? customMessage = null
	) => AddRule(new NotHasFlagRule<TEnum>(forbiddenFlag, customMessage));

	/// <summary>
	///     Adds a custom validation rule to the handler.
	/// </summary>
	/// <param name="predicate">
	///     The function that determines whether an enum value is valid. Cannot be
	///     null.
	/// </param>
	...
	public EnumInputBuilder<TEnum> WithCustomRule(Func<TEnum, bool> predicate, string errorMessage)
	{
		...
		return AddRule(new CustomRule<TEnum>(predicate, errorMessage));
	}
}
