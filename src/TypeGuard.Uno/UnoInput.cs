...
/// <summary>
/// Provides Uno-based input by reading the current text value from a <see cref="TextBox"/>.
/// </summary>
...
public class UnoInput(TextBox textBox) : IInputProvider
{
	...
	/// via <see cref="DispatcherQueue.TryEnqueue(DispatcherQueueHandler)"/>.
	/// </remarks>
	...
	public string? GetInput()
	{
		...
		else
		{
			...
			{
				captured = _textBox.Text;
				tcs.SetResult(captured);
			});
			text = tcs.Task.GetAwaiter().GetResult();
		}
